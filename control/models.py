...
class Transitario(models.Model):
    ...
    def clean(self):
        if self.calificacion and (self.calificacion < 1 or self.calificacion > 5):
            raise ValidationError({'calificacion': 'La calificación debe estar entre 1 y 5'})
...
class Arribo(models.Model):
    ...
    fecha_etd = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="ETD (Estimated Time of Departure)"
    )
    fecha_arribo_real = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Fecha de Arribo Real"
    )
...
class Contenedor(models.Model):
    ...
    mercancia_declarada = models.CharField(
        max_length=255,
        verbose_name="Mercancía Declarada"
    )
    mercancia_peligrosa = models.BooleanField(
        default=False,
        verbose_name="Mercancía Peligrosa"
    )
    ...
    bl_referencia = models.CharField(
        max_length=50,
        verbose_name="BL de Referencia",
        help_text="Bill of Lading"
    )
    ...
    origen_pais = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name="País de Origen/Destino"
    )
    origen_puerto = models.CharField(
        max_length=120,
        null=True,
        blank=True,
        verbose_name="Puerto de Origen/Destino"
    )
    ...
    def esta_liberado_aduana(self):
        ...
        return (hasattr(self, 'aprobacion_aduanera') and
                self.aprobacion_aduanera.aprobado and
                self.aprobacion_aduanera.fecha_levante is not None)

    @property
    def esta_pagado(self):
        """Verifica si el cliente ha pagado por este contenedor (calculado)"""
        return (hasattr(self, 'aprobacion_financiera') and
                self.aprobacion_financiera.fecha_pago is not None)

    @property
    def transitario_ha_pagado(self):
        """Verifica si el transitario ha pagado al puerto (calculado)"""
        return (hasattr(self, 'aprobacion_pago_transitario') and
                self.aprobacion_pago_transitario.pago_realizado)
